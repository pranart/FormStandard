...
namespace FormStandard.iOS
{
	...
	{
		public static int MaximumImagesCount { get; set; }
		...
		public static ELCImagePickerViewController Instance
		{
			get
			{
				...
				ELCImagePickerViewController.MaximumImagesCount = 0;
				picker.NavigationBar.BarStyle = UIBarStyle.Default;
				return picker;
			}
		}
		...
		bool ShouldSelectAsset(ALAsset asset, int previousCount)
		{
			...
			if (!shouldSelect)
			{
				string title = string.Format("คำเตือน", MaximumImagesCount);
				string message = string.Format("เลือกได้ {0} รูป เท่านั้น", MaximumImagesCount);
				var alert = new UIAlertView(title, message, null, null, "ตกลง");
				alert.Show();
			}
			return shouldSelect;
		}
		...
		class ELCAssetTablePicker : UITableViewController
		{
			...
			public static int Gap = 9;

			public override void ViewWillAppear(bool animated)
			{
				base.ViewWillAppear(animated);
				Columns = (int)(View.Bounds.Size.Width / (ELCAssetCell.ImageWidth + Gap));
			}
			...
			void PreparePhotos()
			{
				...
				_Dispatcher.BeginInvokeOnMainThread(() =>
				{
					...
					if (section >= 0 && row >= 0)
					{
						...
						TableView.ScrollToRow(ip, UITableViewScrollPosition.Top, false);
					}
					NavigationItem.Title = "0/" + MaximumImagesCount.ToString();
				});
			}
			...
			bool ShouldSelectAsset(ELCAsset asset)
			{
				...
				if (parent != null)
				{
					...
					NavigationItem.Title = (selectionCount + (shouldSelect ? 1 : 0)).ToString() + "/" + MaximumImagesCount.ToString();
				}
			...
			public override nfloat GetHeightForRow(UITableView tableView, NSIndexPath indexPath)
			{
				return ELCAssetCell.ImageWidth + 10;
			}
			...
			class ELCAssetCell : UITableViewCell
			{
				...
				void CellTapped(UITapGestureRecognizer tapRecognizer)
				{
					...
					var totalWidth = Columns * ImageWidth + (Columns - 1) * Gap;
					var startX = (Bounds.Size.Width - totalWidth) / 2;

					var frame = new CGRect((nfloat)startX, 2, ImageWidth, ImageWidth);
					for (int i = 0; i < RowAssets.Count; ++i)
					{
						...
						var x = frame.X + frame.Width + Gap;
						frame = new CGRect(x, frame.Y, frame.Width, frame.Height);
					}
				}
				public static int ImageWidth = 124;
				public override void LayoutSubviews()
				{
					var totalWidth = Columns * ImageWidth + (Columns - 1) * Gap;
					var startX = (Bounds.Size.Width - totalWidth) / 2;

					var frame = new CGRect(startX, 2, ImageWidth, ImageWidth);
